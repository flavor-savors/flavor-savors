...
class Question extends Component {
  ...
  deleteReply = id => {
    ...
    axios
      .delete(`/forum/reply/${this.state.question.id}`, id)
      .then(() => this.getQuestion(this.state.question.id));
  };
  ...
  render() {
    const question = this.props.questions.map((e, i) => {
      return (
        <div
          key={i}
          onMouseEnter={() => this.getQuestion(e.id)}
          ...
          <div className='user-buttons-name'>
            <div className='username'>
              <p>POSTED BY: {e.username.toUpperCase()}</p>
            </div>
            <button
              onClick={() => this.props.deletePost(e.id)}
